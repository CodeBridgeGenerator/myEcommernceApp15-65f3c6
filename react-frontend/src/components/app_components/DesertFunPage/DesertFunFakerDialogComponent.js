...
import desertFunFakerFactory from "./desertFunFakerFactory";
...
const DesertFunFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = desertFunFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("desertFun").create(elem));
...
export default connect(mapState, mapDispatch)(DesertFunFakerDialogComponent);
